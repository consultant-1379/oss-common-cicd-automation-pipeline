...
pipeline {
    ...
    stages {
        ...
        stage('Bump Common CICD Automation Pipeline Tool Version') {
            steps {
                sh "${bob} set-common-cicd-automation-pipeline-env-variables bump-service-version"
                script {
                    env.IMAGE_VERSION = readFile('artifact.properties').trim()
                }
            }
        }
        stage('Build Common CICD Automation Pipeline Tool') {
            steps {
                sh "${bob} build-docker-image"
            }
        }
        stage('Publish Common CICD Automation Pipeline Tool') {
            steps {
                sh "${bob} publish-docker-image clean-up-docker-image"
            }
        }
        stage('Add changes to Version file of Common CICD Automation Pipeline Tool') {
            steps {
